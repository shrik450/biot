...
[deployment contract](model.md#deployment-contract). Day-two operations are out of scope; the
last section names them.
...
The node allocates one logical UID/GID range to each Biot and passes that range to Podman. Add
matching subordinate UID and GID entries for the service account in `/etc/subuid` and `/etc/subgid`
(or with the host's equivalent account-management tools).

**Root.** Add the entries with the host's account tooling. For example, this gives `biot` a host
range without tying the host's start ID to the node's logical base:

```sh
usermod --add-subuids <host-start>-<host-end> \
  --add-subgids <host-start>-<host-end> biot
```

Choose `<host-end>` so the inclusive host range contains at least the required number of IDs. The
`<host-start>` in this command is the host-assigned subuid start, not
`BIOT_NODE_UID_RANGE_BASE`.

The important rule is:

```
subordinate range size >= BIOT_NODE_UID_RANGE_LIMIT - BIOT_NODE_UID_RANGE_BASE
```

`BIOT_NODE_UID_RANGE_BASE` is a free logical origin used by the node's allocator. It does **not**
have to equal the first subordinate ID assigned to the service account by the host. The host's
subuid start and the node's logical base are separate numbers. For example, the release defaults
use a logical span of `165536 - 100000 = 65536`; a host may assign those 65536 IDs starting at
524288.

Set the related node values deliberately:

| Setting | Meaning |
| --- | --- |
| `BIOT_NODE_UID_RANGE_BASE` (default `100000`) | Logical origin for the allocator; not the host subuid start |
| `BIOT_NODE_UID_RANGE_COUNT` (default `1024`) | IDs reserved by each Biot |
| `BIOT_NODE_UID_RANGE_LIMIT` (default `BASE + 65536`) | Exclusive logical end of the allocator's span |

If the host grants fewer subordinate IDs than `LIMIT - BASE`, allocation or worker startup fails
with an insufficient-UID error. Increasing the absolute base does not fix that; increase the host
range or reduce the logical span instead.
...
values are required by `config/releases/node.exs`:
...
| `BIOT_NODE_BUILDER_IMAGE` | Pinned builder image digest | The release's pinned image in `config/config.exs` |
| `BIOT_NODE_BINARY_CACHE_URLS`, `BIOT_NODE_BINARY_CACHE_KEYS` | Whitespace-separated Nix cache endpoints and trusted keys | The release's defaults in `config/config.exs` (`https://cache.nixos.org` and its key), or the substituters you trust |
...
- the `nodes` page of the web UI, `/nodes`, shows each node's status, platform, capacity, and
  connection;
- `biot nodes` prints the same as a table ending in a `CONNECTION` column that reads `ready`;
...
- renewing or rotating the control-link certificates, beyond the SSH host-key rotation note;
- diagnosing a Biot that will not start;
...
- **The configuration parsed.** `journalctl -u biot-node -b | grep -c 'environment variable'`
  prints `0`.
- **The subordinate range fits the node's span.** With this example's values,
  `[ "$(awk -F: -v u=biot '$1==u{print $3}' /etc/subuid)" -ge "$((589824 - 524288))" ] && echo ok`
  prints `ok`, and the same check against `/etc/subgid` also prints `ok`.
