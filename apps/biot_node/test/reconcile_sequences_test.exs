defmodule Biot.Node.ReconcileSequencesTest do
  ...
  describe "create to running (docs/model.md section 9)" do
    test "walks allocate, initialize, resolve, prepare, install, start, then settles" do
      ...
      nothing_owned = state()
      assert Reconcile.next(spec, nothing_owned, nil, :ready) == {:run, {:allocate, biot_id()}}

      allocated = %{nothing_owned | data: {:uninitialized, fresh_allocation()}}

      assert Reconcile.next(spec, allocated, nil, :ready) ==
               {:run, {:initialize, fresh_allocation(), repository()}}

      initialized = %{allocated | data: {:present, allocation(), marker()}}

      assert Reconcile.next(spec, initialized, nil, :ready) ==
               {:run, {:resolve, e1(), selection(), allocation()}}

      resolved = %{initialized | resolutions: %{e1() => {:present, resolution(e1())}}}

      assert Reconcile.next(spec, resolved, nil, :ready) == {:run, {:prepare, e1(), manifest()}}

      prepared = %{resolved | prepared: {:present, %{e1() => artifact(e1())}}}

      assert Reconcile.next(spec, prepared, nil, :ready) ==
      ...
      assert Reconcile.next(spec, running, nil, :ready) == :settled
    end
    ...
    test "a restarted controller continues from the first unmet condition, never from the clone" do
      ...
      assert Reconcile.next(spec(), recovered, nil, :ready) ==
               {:run, {:start, allocation(), installation(e1())}}
    end
  end
  ...
  describe "stop then start" do
    test "the stop observes absence and the later start creates a new incarnation" do
      ...
      assert Reconcile.next(stopped, settled(e1()), nil, :ready) ==
               {:run, {:retire, incarnation()}}

      absent = %{settled(e1()) | container: :absent}
      assert Reconcile.next(stopped, absent, nil, :ready) == :settled

      started = spec(state: :running, revision: 3)

      assert Reconcile.next(started, absent, nil, :ready) ==
               {:run, {:start, allocation(), installation(e1())}}

      new_incarnation = %{absent | container: container(biot_id(), e1(), :running)}
      assert Reconcile.next(started, new_incarnation, nil, :ready) == :settled
    end
  end
  ...
  describe "update environment" do
    test "prepares, retires, installs after absence, starts, then releases the old environment" do
      ...
      assert Reconcile.next(spec, resolved_both, nil, :ready) ==
      ...
      assert Reconcile.next(spec, prepared_both, nil, :ready) == {:run, {:retire, incarnation()}}
      ...
      assert Reconcile.next(spec, running, nil, :ready) == {:run, {:release_environment, e1()}}
      ...
      assert Reconcile.next(spec, released, nil, :ready) == :settled
    end
  end
  ...
  describe "destroy" do
    test "retires, releases the environment, removes data, releases the allocation, then settles" do
      ...
      assert Reconcile.next(spec, settled(e1()), nil, :ready) == {:run, {:retire, incarnation()}}

      retired = %{settled(e1()) | container: :absent}
      assert Reconcile.next(spec, retired, nil, :ready) == {:run, {:release_environment, e1()}}

      released = %{retired | prepared: :absent, resolutions: %{}}
      assert Reconcile.next(spec, released, nil, :ready) == {:run, {:remove_data, allocation()}}

      removed = %{released | data: {:uninitialized, fresh_allocation()}, installation: nil}

      assert Reconcile.next(spec, removed, nil, :ready) ==
               {:run, {:release_allocation, fresh_allocation()}}

      gone = %{removed | data: :no_allocation}
      assert Reconcile.next(spec, gone, nil, :ready) == :settled
    end
    ...
    test "remove_data converges over a completion marker whose data are already gone" do
      ...
      assert Reconcile.next(spec, lost, nil, :ready) == {:run, {:remove_data, allocation()}}
    end
  end
end
